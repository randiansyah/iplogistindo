...
class List_delivery extends Admin_Controller {
  ...
  public function index()
  {
    ...
    if($this->data['is_can_read']){
      $this->data['content'] = 'admin/list_delivery/list_v';   
    }else{
 ...
 //update data delivery
     ...
     $update = $this->transaksi_model->update($dataD,array("id_transaksi"=>$id));
      ...
      if ($update)
      {
        ...
        $this->session->set_flashdata('message', "Delivery Berhasil Di konfirmasi");
        redirect("List_delivery");


      }else{
        $this->session->set_flashdata('message_error', "Delivery gagal di konfirmasi ");
        redirect("List_delivery","refresh");
      }
    }else{
      if(!empty($_POST)){ 
        $this->session->set_flashdata('message_error',validation_errors());
        return redirect("List_delivery/proses/".$id);  
      }else{
    ...
    { 
      ...
      if(!empty($_POST)){ 
       ...
       $this->data['delivery'] =   (!empty($data))?$data[0]->delivery:"";
        ...
        $this->data['content'] = 'admin/list_delivery/detail_v'; 
        $this->data['pelanggan'] = $this->customer_model->getAllById();
        $this->data['data_provinsi'] = $this->wilayah_model->getAllProvince();  
        $this->data['vendor'] = $this->vendor_model->getAllById();   
